...
`nosh` is a [Bourne Shell Emulator](#Bourne_Shell_Emulator), [Erl Shell Alternative](#Erl_Shell_Alternative)
 and [NoSQL File System](#NoSQL_File_System) implemented in Erlang.  Providing an intuitive
...
_Full terminal emulation has yet to be implemented._  The `nosh` process continues to
 ...
 not be confused with the [Erlang Context](#Erlang_Context) left-wise conjunction; or with the period
 command, one of the built-in [Bourne Commands](#Bourne_Commands).)
...
`nosh` emulates much of the functionality of the Bourne shell (`sh`), including [Quoting](#Quoting) and [Grouping](#Grouping).
...
Two syntax regimes are supported by `nosh`, identified as Bourne Context and [Erlang Context](#Erlang_Context).
 Bourne Context is the emulation of `sh` syntax, marked by [Grouping](#Grouping) rules and parameters 
...
For a discussion of how Erlang modules and functions are used in the Bourne Context, see the
 [Erl Shell Alternative](#Erl_Shell_Alternative) section.
...
Double quotes remove the special meaning of all characters other than the dollar sign (`$`), the
 back quote (```), the backslash (`\`), and the newline (per [Multiline Parsing](#Multiline_Parsing)).
 Additionally, double quotes indicate the `string()` type, _i.e._ a `list()` of characters.
 Strings are passed to Erlang functions only after all embedded [Parameter Expansion](#Parameter_Expansion),
 [Command Substition](#Command_Substition) and [Arithmetic Expansion](#Arithmetic_Expansion) has been performed.

 

Back quotes signal [Command Substitution](#Command_Substitution), the back quoted character sequence being replaced
 by the results of the substituted command(s) execution.  A [Lists](#Lists) back quote (`[`...`]`)
...
In first position, parentheses (`(...)`) group commands within for execution in a [Subshell Environment](#Subshell_Environment),
 as per [Command Substitution](#Command_Substitution).  A parentheses group appearing in second position, following
 a command, is treated as an [Parameter List](#Parameter_List).  Parentheses groups
...
The faux within-shell execution syntax of Bourne shell (`{ ...; };`)--implemented
 ...
 supported by `nosh`, curly brackets instead marking [Tuples](#Tuples).
...
These [Bourne Context](#Bourne_Context) conjunctions are parsed right-wise.  That is, everything that follows the conjunction
 is returned as a nested group in the last element of the enclosing group context.  There are two left-wise conjunctions,
 the period (`.`), indicating an [Erlang Context](#Erlang_Context) group, and the `<newline>`, 
...
As per Bourne shell, pipes are marked by a single vertical bar (`|`).  Erlang processes may participate in pipe
 relationships if they implement the [Nosh_exec Behaviour](#Nosh_exec_Behaviour).  Otherwise, processes grouped by the single vertical bar
 will run, but each in an isolated [Subshell Environment](#Subshell_Environment) without any access to piped standard input and output streams.
...
For these reasons, `nosh` operates in [Bourne Context](#Bourne_Context) by default, while still supporting
 Erlang [Lists](#Lists) and [Tuples](#Tuples).  Within Bourne Context, [Modules](#Modules) compile
 and load on-the-fly, [Parameters](#Parameters) may be passed (and functions called) with minimal typing,
 and [Exceptions](#Exceptions) thrown by processes are gracefully caught and displayed.
...
If an associated `.erl` file is found, and it is newer that the `.beam` file, or if an `.erl` file
 ...
 directory.  If this compilation is successful, the module will be loaded and [Execution](#Execution) proceeds.
 Otherwise, the compiler error is written to `stdout` and a non-zero status is returned.

 

If no associated `.erl` file is found, the `.beam` file on the $PATH is loaded and [Execution](#Execution)
...
In _implicit start_ mode, the module name appears in first position (as a command) with or without parameters.
 ...
 or else `start/3`, as specified by the [Nosh_exec Behaviour](#Nosh_exec_Behaviour), and failing that,
...
If with parameters, `nosh` will first call any getoptions functions, caching the results from each.
 ...
 module's [Nosh_exec Behaviour](#Nosh_exec_Behaviour) `start/6` function, reporting an error on failure.  Otherwise,
...
In _explicit function_ mode, the command appears in Erlang syntax as _`Module`_`:`_`Function`_.
 This can stand on it's own, or else be followed either by [Parameters](#Parameters) or a [Parameter List](#Parameter_List).
 The resulting arity function is called by `nosh`, reporting an error on failure.  In [Erlang Context](#Erlang_Context), 
...
In _getoptions functions_ mode, the module name appears in first position, followed by
 [Parameters](#Parameters), including one or more 0-arity and/or 1-arity function names specified in Perl getoptions
...
Parameters may be Bourne-style [Words](#Words), including character sequences delimited by single or double
 [Quoting](#Quoting).  In addition, Erlang-syntax [Lists](#Lists) and/or [Tuples](#Tuples) may be
 specified as parameters.  [Parameter List](#Parameter_List) syntax is a special case only valid in
 explicit function [Execution](#Execution) mode.
...
Zero or more Words, Lists and/or Tuples are treated as a single List parameter
 (not to be confused with a [Parameter List](#Parameter_List)) when enclosed by an open square bracket (`[`)
...
[Command Substitution](#Command_Substitution) may be used to generate an Erlang List, by nesting back quotes
...
A Parameter List follows Erlang syntax for specifying the parameters (and indirectly, the arity) of a
 ...
 may only appear in second position after a command, and only in explicit function mode (see [Execution](#Execution)).

 

In first position, parentheses denote command [Grouping](#Grouping).
...
The `nosh` process and/or [Subshell Environment](#Subshell_Environment) that calls or spawns any given module function
 ...
 status is passed to any conditional [Grouping](#Grouping).
...
_Not yet implemented:_  The Erlang Context is a special case of [Grouping](#Grouping), denoted by the left-wise
 conjunction, period (`.`).  When the `nosh` command parser encounters a period on it's own (not as part of any
 [Parameters](#Parameters)), it rolls everything in the current context to the left of the period into a sub-context, 
...
The period conjunction should not be confused with either the [Noterm](#Noterm) end-of-file character or the period
 command, one of the built-in [Bourne Commands](#Bourne_Commands).
