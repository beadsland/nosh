...
executing only on a zero status (or an Erlang return value of`ok` or`{ok, ...}`), and the command group following a double vertical bar  
...
There are two left-wise conjunctions, the period (`.`), marking an[Erlang Context](#Erlang_Context), and the`<newline>`, marking the top-level  
...
Strings are passed to Erlang functions only after all embedded[Substitution Contexts](#Substitution_Contexts) have been evaluated.
...
The[Lists](#Lists) back quote (`[`...`]`) and[Tuples](#Tuples) back quote (`{`...`}`) are two special
...
or more word contexts, each being delimited by the characters in the`IFS` environment variable (or`<space>`,`<tab>` and`<newline>`
...
_Not yet implemented_.<a name="types"></a>
...
<pre>term_type() =word |list |plst |tupl |epid |bstr</pre><a name="index"></a>
