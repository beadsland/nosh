...
`nosh` is a [Bourne Shell Emulator](http://github.com/beadsland/nosh/blob/master/doc/#Bourne_Shell_Emulator), [Erl Shell Alternative](http://github.com/beadsland/nosh/blob/master/doc/#Erl_Shell_Alternative)
 and [NoSQL File System](http://github.com/beadsland/nosh/blob/master/doc/#NoSQL_File_System) implemented in Erlang.  Providing an intuitive
 ...
 * [Noterm](http://github.com/beadsland/nosh/blob/master/doc/#Noterm)

 * [Bourne Shell Emulator](http://github.com/beadsland/nosh/blob/master/doc/#Bourne_Shell_Emulator)

 * [Erl Shell Alternative](http://github.com/beadsland/nosh/blob/master/doc/#Erl_Shell_Alternative)

 * [NoSQL File System](http://github.com/beadsland/nosh/blob/master/doc/#NoSQL_File_System)
...
_Full terminal emulation has yet to be implemented._  The `nosh` process continues to
 ...
 not be confused with the [Erlang Context](http://github.com/beadsland/nosh/blob/master/doc/#Erlang_Context) left-wise conjunction; or with the period
 command, one of the built-in [Bourne Commands](http://github.com/beadsland/nosh/blob/master/doc/#Bourne_Commands).)
...
`nosh` emulates much of the functionality of the Bourne shell (`sh`), including [Quoting](http://github.com/beadsland/nosh/blob/master/doc/#Quoting) and [Grouping](http://github.com/beadsland/nosh/blob/master/doc/#Grouping).
...
Two syntax regimes are supported by `nosh`, identified as Bourne Context and [Erlang Context](http://github.com/beadsland/nosh/blob/master/doc/#Erlang_Context).
 Bourne Context is the emulation of `sh` syntax, marked by [Grouping](http://github.com/beadsland/nosh/blob/master/doc/#Grouping) rules and parameters 
...
For a discussion of how Erlang modules and functions are used in the Bourne Context, see the
 [Erl Shell Alternative](http://github.com/beadsland/nosh/blob/master/doc/#Erl_Shell_Alternative) section.
...
Double quotes remove the special meaning of all characters other than the dollar sign (`$`), the
 back quote (```), the backslash (`\`), and the newline (per [Multiline Parsing](http://github.com/beadsland/nosh/blob/master/doc/#Multiline_Parsing)).
 Additionally, double quotes indicate the `string()` type, _i.e._ a `list()` of characters.
 Strings are passed to Erlang functions only after all embedded [Parameter Expansion](http://github.com/beadsland/nosh/blob/master/doc/#Parameter_Expansion),
 [Command Substition](http://github.com/beadsland/nosh/blob/master/doc/#Command_Substition) and [Arithmetic Expansion](http://github.com/beadsland/nosh/blob/master/doc/#Arithmetic_Expansion) has been performed.

 

Back quotes signal [Command Substitution](http://github.com/beadsland/nosh/blob/master/doc/#Command_Substitution), the back quoted character sequence being replaced
 by the results of the substituted command(s) execution.  A [Lists](http://github.com/beadsland/nosh/blob/master/doc/#Lists) back quote (`[`...`]`)
...
In first position, parentheses (`(...)`) group commands within for execution in a [Subshell Environment](http://github.com/beadsland/nosh/blob/master/doc/#Subshell_Environment),
 as per [Command Substitution](http://github.com/beadsland/nosh/blob/master/doc/#Command_Substitution).  A parentheses group appearing in second position, following
 a command, is treated as an [Parameter List](http://github.com/beadsland/nosh/blob/master/doc/#Parameter_List).  Parentheses groups
...
The faux within-shell execution syntax of Bourne shell (`{ ...; };`)--implemented
 ...
 supported by `nosh`, curly brackets instead marking [Tuples](http://github.com/beadsland/nosh/blob/master/doc/#Tuples).
...
These [Bourne Context](http://github.com/beadsland/nosh/blob/master/doc/#Bourne_Context) conjunctions are parsed right-wise.  That is, everything that follows the conjunction
 is returned as a nested group in the last element of the enclosing group context.  There are two left-wise conjunctions,
 the period (`.`), indicating an [Erlang Context](http://github.com/beadsland/nosh/blob/master/doc/#Erlang_Context) group, and the `<newline>`, 
...
As per Bourne shell, pipes are marked by a single vertical bar (`|`).  Erlang processes may participate in pipe
 relationships if they implement the [Nosh_exec Behaviour](http://github.com/beadsland/nosh/blob/master/doc/#Nosh_exec_Behaviour).  Otherwise, processes grouped by the single vertical bar
 will run, but each in an isolated [Subshell Environment](http://github.com/beadsland/nosh/blob/master/doc/#Subshell_Environment) without any access to piped standard input and output streams.
...
For these reasons, `nosh` operates in [Bourne Context](http://github.com/beadsland/nosh/blob/master/doc/#Bourne_Context) by default, while still supporting
 Erlang [Lists](http://github.com/beadsland/nosh/blob/master/doc/#Lists) and [Tuples](http://github.com/beadsland/nosh/blob/master/doc/#Tuples).  Within Bourne Context, [Modules](http://github.com/beadsland/nosh/blob/master/doc/#Modules) compile
 and load on-the-fly, [Parameters](http://github.com/beadsland/nosh/blob/master/doc/#Parameters) may be passed (and functions called) with minimal typing,
 and [Exceptions](http://github.com/beadsland/nosh/blob/master/doc/#Exceptions) thrown by processes are gracefully caught and displayed.
...
If an associated `.erl` file is found, and it is newer that the `.beam` file, or if an `.erl` file
 ...
 directory.  If this compilation is successful, the module will be loaded and [Execution](http://github.com/beadsland/nosh/blob/master/doc/#Execution) proceeds.
 Otherwise, the compiler error is written to `stdout` and a non-zero status is returned.

 

If no associated `.erl` file is found, the `.beam` file on the $PATH is loaded and [Execution](http://github.com/beadsland/nosh/blob/master/doc/#Execution)
...
In _implicit start_ mode, the module name appears in first position (as a command) with or without parameters.
 ...
 or else `start/3`, as specified by the [Nosh_exec Behaviour](http://github.com/beadsland/nosh/blob/master/doc/#Nosh_exec_Behaviour), and failing that,
...
If with parameters, `nosh` will first call any getoptions functions, caching the results from each.
 ...
 module's [Nosh_exec Behaviour](http://github.com/beadsland/nosh/blob/master/doc/#Nosh_exec_Behaviour) `start/6` function, reporting an error on failure.  Otherwise,
...
In _explicit function_ mode, the command appears in Erlang syntax as _`Module`_`:`_`Function`_.
 This can stand on it's own, or else be followed either by [Parameters](http://github.com/beadsland/nosh/blob/master/doc/#Parameters) or a [Parameter List](http://github.com/beadsland/nosh/blob/master/doc/#Parameter_List).
 The resulting arity function is called by `nosh`, reporting an error on failure.  In [Erlang Context](http://github.com/beadsland/nosh/blob/master/doc/#Erlang_Context), 
...
In _getoptions functions_ mode, the module name appears in first position, followed by
 [Parameters](http://github.com/beadsland/nosh/blob/master/doc/#Parameters), including one or more 0-arity and/or 1-arity function names specified in Perl getoptions
...
Parameters may be Bourne-style [Words](http://github.com/beadsland/nosh/blob/master/doc/#Words), including character sequences delimited by single or double
 [Quoting](http://github.com/beadsland/nosh/blob/master/doc/#Quoting).  In addition, Erlang-syntax [Lists](http://github.com/beadsland/nosh/blob/master/doc/#Lists) and/or [Tuples](http://github.com/beadsland/nosh/blob/master/doc/#Tuples) may be
 specified as parameters.  [Parameter List](http://github.com/beadsland/nosh/blob/master/doc/#Parameter_List) syntax is a special case only valid in
 explicit function [Execution](http://github.com/beadsland/nosh/blob/master/doc/#Execution) mode.
...
The simplest `nosh` command line consists of a command followed by zero or more words, this
 ...
 (see [Quoting](http://github.com/beadsland/nosh/blob/master/doc/#Quoting)).
...
Zero or more Words, Lists and/or Tuples are treated as a single List parameter
 (not to be confused with a [Parameter List](http://github.com/beadsland/nosh/blob/master/doc/#Parameter_List)) when enclosed by an open square bracket (`[`)
...
[Command Substitution](http://github.com/beadsland/nosh/blob/master/doc/#Command_Substitution) may be used to generate an Erlang List, by nesting back quotes
...
A Parameter List follows Erlang syntax for specifying the parameters (and indirectly, the arity) of a
 ...
 may only appear in second position after a command, and only in explicit function mode (see [Execution](http://github.com/beadsland/nosh/blob/master/doc/#Execution)).

 

In first position, parentheses denote command [Grouping](http://github.com/beadsland/nosh/blob/master/doc/#Grouping).
...
The `nosh` process and/or [Subshell Environment](http://github.com/beadsland/nosh/blob/master/doc/#Subshell_Environment) that calls or spawns any given module function
 ...
 status is passed to any conditional [Grouping](http://github.com/beadsland/nosh/blob/master/doc/#Grouping).
...
_Not yet implemented:_  The Erlang Context is a special case of [Grouping](http://github.com/beadsland/nosh/blob/master/doc/#Grouping), denoted by the left-wise
 conjunction, period (`.`).  When the `nosh` command parser encounters a period on it's own (not as part of any
 [Parameters](http://github.com/beadsland/nosh/blob/master/doc/#Parameters)), it rolls everything in the current context to the left of the period into a sub-context, 
...
The period conjunction should not be confused with either the [Noterm](http://github.com/beadsland/nosh/blob/master/doc/#Noterm) end-of-file character or the period
 command, one of the built-in [Bourne Commands](http://github.com/beadsland/nosh/blob/master/doc/#Bourne_Commands).
