...
%% @doc This is a preliminary draft of the command line execution module
%% for `nosh'.
%%
%% <b>Draft Notes:</b>
%%
%% <i>Now thinking these multiple `start' functions will be supeceded
%% by simpler `run' functions.  Still thinking it through.</i>
%%
%% Module execution may be invoked in one of three modes:  implicit start,
%% explicit function or getoptions functions.
%%
%% In <i>implicit start</i> mode, the module name appears in first
...
%% error on failure.
%%
%% In <i>explicit function</i> mode, the command appears in Erlang syntax
%% as <i>`Module'</i>`:'<i>`Function'</i>.  This can stand on it's own, or
